...
public class QueryGenerator {
	...
	public QueryGenerator(List<Integer> integerNumbers) {
	...
	public List<SingleQuery> getSingleQueries() {
		...
		// First Element.
		result.add(new SingleQuery(min, true));
		// Last Element.
		result.add(new SingleQuery(max, true));
		// Mid Element.
		result.add(new SingleQuery(sortedNumbers.elementAt(sortedNumbers.size() / 2), true));
		// Mid range element.
		final int midRange = (max - min) / 2;
		result.add(new SingleQuery(closest(midRange), true));
		// Miss first.
		int i = min + 1;
		while(Collections.binarySearch(sortedNumbers, i) >= 0) {
			i++;
		}
		result.add(new SingleQuery(i, false));
		// Miss last.
		i = max - 1;
		while(Collections.binarySearch(sortedNumbers, i) >= 0) {
			i--;
	...
	public List<RangeQuery> getRangeQueries() {
		...
		// Full Range.
		result.add(new RangeQuery(min, max, getResultSize(min, max)));
		// Range without upper and lower.
		int i,j;
		i = min + 1;
		j = max - 1;
		result.add(new RangeQuery(i, j, getResultSize(i, j)));
		// Min to mid range.
		i = min;
		j = (max - min) / 2;
		result.add(new RangeQuery(i, j, getResultSize(i, j)));
