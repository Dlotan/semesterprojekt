...
public class DiceMaster {
	...
	public DiceMaster(RandomGenerator generator, int num_randomRangeClasses,
			int num_randomNumbers) {
			...
			if (randomRangeClasses.elementAt(i).getValues().size() < 5) {
				...
				for (; j < randomRangeClasses.size(); j++) {
					if (randomRangeClasses.elementAt(j).getValues().size() == 0) {
						...
						randomRangeClasses.elementAt(j).addRandomRange(
								randomRangeClasses.elementAt(i));
						break;
					}
				}
